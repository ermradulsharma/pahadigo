...
class VendorController extends Controller {
    ...
    async getVendorById(req, { params }) {
        try {
            if (!params.id) throw new AppError(RESPONSE_MESSAGES.VALIDATION.ID_REQUIRED, HTTP_STATUS.BAD_REQUEST);
    ...
    async createVendor(req) {
        try {
            const validation = validate(schemas.vendorCreate, req.payload || {});
            if (!validation.success) throw new AppError(validation.error, HTTP_STATUS.BAD_REQUEST);
            
            const result = await VendorService.createVendor(validation.data, req);
    ...
    async updateVendor(req, { params }) {
        try {
            const validation = validate(schemas.vendorUpdate, req.payload || {});
            if (!validation.success) throw new AppError(validation.error, HTTP_STATUS.BAD_REQUEST);

            const vendor = await VendorService.updateVendor(params.id, validation.data, req);
    ...
    async approveVendor(req) {
        try {
            const validation = validate(schemas.adminVendorApproval, req.payload || {});
            if (!validation.success) throw new AppError(validation.error, HTTP_STATUS.BAD_REQUEST);

            await VendorService.updateVendorStatus(validation.data.vendorId, validation.data.status, req);
    ...
    async verifyDocument(req) {
        try {
            const validation = validate(schemas.documentVerification, req.payload || {});
            if (!validation.success) throw new AppError(validation.error, HTTP_STATUS.BAD_REQUEST);

            await VendorService.verifyManualDocument(validation.data, req);
