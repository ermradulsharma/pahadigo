...
class VendorController {
    ...
    async createBusinessProfile(req) {
        try {
            const user = req.user;
            const statusCheck = await VendorStatusService.canManageBusinessProfile(user.id);
            if (!statusCheck.allowed) return errorResponse(HTTP_STATUS.FORBIDDEN, statusCheck.message, {});
    ...
    // PATCH /vendor/business/status
    async updateBusinessStatus(req) {
        try {
            const user = req.user;
            let body;
            if (req.formDataBody) {
    ...
    // GET /vendor/business/profile/closures
    ...
    // POST /vendor/business/profile/closures
    async addClosurePeriod(req) {
        try {
            const user = req.user;
            let body;
            if (req.formDataBody) {
    ...
    // PATCH /vendor/business/profile/closures/:id
    async updateClosurePeriod(req, { params }) {
        try {
            const user = req.user;
            const { id } = params;
            let body;
    ...
    // DELETE /vendor/business/profile/closures/:id
    async deleteClosurePeriod(req, { params }) {
        try {
            const user = req.user;
            const { id } = params;
    ...
    async createPackage(req) {
        try {
            const user = req.user;
            const statusCheck = await VendorStatusService.isVendorAllowedToOperate(user.id);
            if (!statusCheck.allowed) return errorResponse(HTTP_STATUS.FORBIDDEN, statusCheck.message, {});

            const vendor = statusCheck.vendor;
