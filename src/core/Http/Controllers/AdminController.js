...
class AdminController {
  ...
  async getVendors(req) {
    try {
      ...
      console.error("DEBUG getAllVendors ERROR:", error);
      return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, RESPONSE_MESSAGES.ERROR.SERVER_ERROR, { debug: error.message });
    }
  }
  ...
  async getVendorById(req, { params }) {
    try {
      ...
      if (error.message === "Vendor node not found") return errorResponse(HTTP_STATUS.NOT_FOUND, error.message, {});
      return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, RESPONSE_MESSAGES.ERROR.SERVER_ERROR, {});
    }
  }
  ...
  async getVendorPackages(req, { params }) {
    try {
      const { id } = params;
      if (!id) return errorResponse(HTTP_STATUS.BAD_REQUEST, RESPONSE_MESSAGES.VALIDATION.ID_REQUIRED, {});
      const packages = await AdminService.getVendorPackages(id);
      return successResponse(HTTP_STATUS.OK, RESPONSE_MESSAGES.SUCCESS.FETCHED, { packages });
    } catch (error) {
      return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, RESPONSE_MESSAGES.ERROR.SERVER_ERROR, {});
    }
  }
  ...
  // DELETE /admin/travellers/:id
  async deleteTraveller(req, { params }) {
    try {
      const { id } = params;
      if (!id) return errorResponse(HTTP_STATUS.BAD_REQUEST, RESPONSE_MESSAGES.VALIDATION.ID_REQUIRED, {});
      await AdminService.deleteTraveller(id);
      return successResponse(HTTP_STATUS.OK, "Traveller deleted successfully", {});
    } catch (error) {
      return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, error.message || RESPONSE_MESSAGES.ERROR.SERVER_ERROR, {});
    }
  }
  ...
  async deleteVendor(req, { params }) {
    try {
      ...
      return successResponse(HTTP_STATUS.OK, "Vendor deleted successfully", {});
    } catch (error) {
      return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, error.message || RESPONSE_MESSAGES.ERROR.SERVER_ERROR, {});
    }
  }
  ...
  async getPackageItem(req, { params }) {
    try {
      const item = await AdminService.getPackageItem(params.id);
      return successResponse(HTTP_STATUS.OK, RESPONSE_MESSAGES.PACKAGE.FETCHED || "Item fetched", { item });
    } catch (error) {
  ...
  async updatePackageItem(req, { params }) {
    try {
      const body = req.jsonBody || await req.json();
      const updatedItem = await AdminService.updatePackageItem(params.id, body || {});
      return successResponse(HTTP_STATUS.OK, RESPONSE_MESSAGES.SUCCESS.UPDATED || "Item updated successfully", { item: updatedItem });
    } catch (error) {
      const status = error.message === "Item not found" ? HTTP_STATUS.NOT_FOUND : HTTP_STATUS.INTERNAL_SERVER_ERROR;
  ...
  async getAuditLogs(req) {
    try {
      ...
      const userId = url.searchParams.get('userId') || url.searchParams.get('adminId'); // support legacy param
      const action = url.searchParams.get('action');
      const target = url.searchParams.get('target');
      const startDate = url.searchParams.get('startDate');
      const page = parseInt(url.searchParams.get('page') || '1');
      const limit = parseInt(url.searchParams.get('limit') || '20');
      const result = await AdminService.getAuditLogs({ userId, action, target, startDate }, page, limit);
      return successResponse(HTTP_STATUS.OK, RESPONSE_MESSAGES.ADMIN.AUDIT_LOGS_FETCHED, result);
    } catch (e) {
      console.error("[AdminController.getAuditLogs] Error:", e);
      return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, RESPONSE_MESSAGES.ERROR.SERVER_ERROR, {});
    }
  }
  ...
  async getDisputes(req) {
    try {
      ...
      const page = parseInt(url.searchParams.get('page') || '1');
      const limit = parseInt(url.searchParams.get('limit') || '20');
      const result = await AdminService.getDisputes({ status, vendorId }, page, limit);
      return successResponse(HTTP_STATUS.OK, RESPONSE_MESSAGES.SUCCESS.FETCHED, result);
    } catch (e) { return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, RESPONSE_MESSAGES.ERROR.SERVER_ERROR, {}); }
  }
  ...
  async resolveDispute(req, { params }) {
    try {
      ...
      const body = req.jsonBody || await req.json();
      if (!body.decision || !body.adminNotes) {
        return errorResponse(HTTP_STATUS.BAD_REQUEST, RESPONSE_MESSAGES.VALIDATION.REQUIRED_FIELDS, {});
      }
      if (!['investigating', 'resolved_refunded', 'resolved_rejected'].includes(body.decision)) {
        return errorResponse(HTTP_STATUS.BAD_REQUEST, 'Invalid decision status.', {});
      }
      const dispute = await AdminService.resolveDispute(adminId, disputeId, body.decision, body.adminNotes, req);
      return successResponse(HTTP_STATUS.OK, 'Dispute updated successfully.', { dispute });
    } catch (e) { return errorResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, e.message || RESPONSE_MESSAGES.ERROR.SERVER_ERROR, {}); }
  }
}
