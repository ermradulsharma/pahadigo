...
class DashboardService {
    ...
    async getDashboardStats() {
        ...
        const [users, totalVendors, pendingVendors, categories, bookings, confirmedBookings, packageStats, dbStats] = await Promise.all([
            ...
            Booking.aggregate([
                { $match: { paymentStatus: { $in: ['paid', 'PAID', 'Paid'] } } },
        ...
        const departuresRaw = await Booking.find({
            ...
            status: 'confirmed'
        }).populate('user', 'name').sort({ startDate: 1 }).limit(5).lean();
        ...
        const result = {
            ...
            categories,
            revenue,
            recentBookings: await Booking.find().populate('user', 'name').populate('package', 'title').sort({ createdAt: -1 }).limit(50).lean(),
            recentVendors: await Vendor.find().populate('user', 'name email').sort({ createdAt: -1 }).limit(5).lean(),
            ...
            systemActivity,
            systemHealth: {
                dbLoad: Math.round((process.memoryUsage().rss / (os.totalmem() / 10)) * 100),
                latency: latency,
    ...
    async getAnalyticsData(period = 'monthly') {
        ...
        const [revenueDataRaw, bookingStatus, userGrowthRaw, topVendors] = await Promise.all([
            Booking.aggregate([
                { $match: { createdAt: { $gte: startDate }, paymentStatus: { $in: ['paid', 'PAID', 'Paid'] } } },
    ...
    async getCalendarEvents(start, end) {
        const bookings = await Booking.find({
            startDate: { $gte: start },
            endDate: { $lte: end }
        }).populate('user', 'name').populate('vendor', 'businessName').lean();
