...
class PackageService {
  ...
  async addItem(userId, vendorId, category, itemData) {
    ...
    if (newItemDoc.details && newItemDoc.details.startPoint) {
      mapToGeoJSON(newItemDoc.details.startPoint);
    }
    if (newItemDoc.details && newItemDoc.details.endPoint) {
      mapToGeoJSON(newItemDoc.details.endPoint);
    }
  ...
  async updateItem(userId, vendorId, category, itemId, updates) {
    ...
    if (item.details && item.details.startPoint) {
      mapToGeoJSON(item.details.startPoint);
    }
    if (item.details && item.details.endPoint) {
