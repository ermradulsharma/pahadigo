...
export default function CyberTable({
        ...
        return data.map((item, index) => ({ item, originalIndex: index })).filter(({ item }) => {
            if (searchKeys.length > 0) {
                ...
                return columns.some(col => {
                    if (!col.accessor && !col.getValue) return false;
                    let val = col.getValue ? col.getValue(item) : item[col.accessor];
                    return String(val || '').toLowerCase().includes(lowerQuery);
                });
            }
        });
    }, [data, searchQuery, searchKeys, columns, totalItems]);

    const itemsCount = totalItems !== null ? totalItems : filteredData.length;
    ...
    const exportToCSV = () => {
        ...
        const exportCols = columns.filter((col, i) => col.exportOnly || !hiddenColIndices.has(i));
        ...
        const rows = dataToExport.map(row => {
            return exportCols.map(col => {
                let cellData = col.getValue ? col.getValue(row) : (col.accessor ? row[col.accessor] : '');
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-black/40 border-b border-white/5 text-slate-400 uppercase text-[10px] font-mono tracking-widest transition-colors">
                            <tr>
                                ...
                                {columns.map((col, idx) => {
                                    if (hiddenColIndices.has(idx) || col.exportOnly) return null;
                                        ...
                                        <React.Fragment key={item._id || originalIndex}>
                                            <tr className={`hover:bg-white/[0.04] transition-colors group ${isSelected ? 'bg-indigo-500/5' : ''}`}>
                                                ...
                                                {columns.map((col, colIndex) => {
                                                    if (hiddenColIndices.has(colIndex) || col.exportOnly) return null;
                                                    return (
