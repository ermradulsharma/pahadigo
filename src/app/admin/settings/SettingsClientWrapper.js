...
import { Settings as SettingsIcon, Mail, BellRing, Smartphone, Key, CreditCard, Bug, Database, ShieldCheck, Fingerprint, AppWindow, Save, CheckCircle, AlertTriangle, Eye, EyeOff, Cloud, Lock, X, Percent } from 'lucide-react';
...
const Input = ({ type = "text", placeholder, className = "", ...props }) => (<input type={type} autoComplete={type === 'password' ? 'new-password' : 'off'} className={`w-full bg-black/40 border border-white/10 rounded-lg px-4 py-2.5 text-sm outline-none transition-all placeholder:text-slate-600 font-mono tracking-wide text-cyan-50 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 focus:bg-white/5 ${className}`} placeholder={placeholder} {...props} />);
...
export default function SettingsClientWrapper({ initialSettings }) {
    ...
    const initialFormData = {
        ...
        razorpay_key_secret: '', razorpay_webhook_secret: '', mongodb_uri: '', api_url: '', jwt_secret: '', google_client_id: '',
    ...
    Object.keys(initialFormData).forEach(key => {
        if (initialSettings[key] !== undefined && initialSettings[key] !== null) {
            initialFormData[key] = initialSettings[key];
        }
    });
    ...
    const sections = [
        ...
        },
        {
            ...
            fields: [
                ...
                { name: 'razorpay_webhook_secret', label: 'Webhook Auth Secret', type: 'password', placeholder: 'Webhook Secret...' }
            ]
        },
        {
            id: 'Infrastructure',
            ...
            icon: Database,
            keys: ['mongodb_uri', 'api_url', 'debug_mode'],
            fields: [
                ...
                { name: 'debug_mode', label: 'System Diagnostics Mode', type: 'checkbox', placeholder: 'Enable Developer Diagnostics' }
            ]
        },
        {
            id: 'UpstashRedis',
            title: 'Upstash Redis Configuration',
            icon: Database,
            keys: ['upstash_redis_rest_url', 'upstash_redis_rest_token', 'upstash_redis_url'],
            fields: [
                { name: 'upstash_redis_rest_url', label: 'Upstash REST API URL', placeholder: 'https://...' },
                { name: 'upstash_redis_rest_token', label: 'Upstash REST Token', type: 'password', placeholder: 'Secret Token...' },
                { name: 'upstash_redis_url', label: 'Upstash Redis URL (TCP)', type: 'password', placeholder: 'rediss://...' }
            ]
        },
        {
            id: 'VercelRedis',
            title: 'Vercel Official Redis',
            icon: Database,
            keys: ['pahadigo_redis_url'],
            fields: [
                { name: 'pahadigo_redis_url', label: 'Vercel Redis URL (TCP)', type: 'password', placeholder: 'redis://...' }
            ]
        },
        {
            id: 'Tokens',
        ...
        },
        {
            ...
            icon: AppWindow,
            keys: ['app_name', 'terms_conditions', 'privacy_policy', 'rate_on_apple_store', 'rate_on_google_store'],
            fields: [
                { name: 'app_name', label: 'Platform Display Name', placeholder: 'PahadiGo' },
                { name: 'terms_conditions', label: 'Terms & Conditions URL', placeholder: 'https://domain.com/terms' },
                { name: 'privacy_policy', label: 'Privacy Policy URL', placeholder: 'https://domain.com/privacy' },
                { name: 'rate_on_apple_store', label: 'Apple App Store URL', placeholder: 'https://apps.apple.com/...' },
                { name: 'rate_on_google_store', label: 'Google Play Store URL', placeholder: 'https://play.google.com/...' }
            ]
        },
        {
            id: 'Secrets',
            ...
            icon: Lock,
            keys: ['social_pass', 'other_account_pass', 'master_otp', 'cron_secret'],
            fields: [
                ...
                { name: 'cron_secret', label: 'CRON Job Secret', type: 'password', placeholder: 'Vercel Cron Secret' }
            ]
        }
    ];
    ...
    const renderField = (field, isPasswordData = false) => {
            ...
            return (
                <FormGroup label={field.label} key={field.name}>
                    <div className="flex items-center mt-2">
                        <input type="checkbox" name={field.name} checked={!!val} onChange={onChange} className="w-4 h-4 text-cyan-500 bg-black/40 border-white/10 rounded focus:ring-cyan-500/20 focus:ring-2 cursor-pointer" />
                        <span className="ml-3 text-sm font-mono text-cyan-50 select-none cursor-pointer" onClick={() => onChange({ target: { name: field.name, type: 'checkbox', checked: !val } })}>{field.placeholder || "Enable"}</span>
                    </div>
                </FormGroup>
            );
        }
        ...
        return (
            <FormGroup label={field.label} key={field.name}>
                <Input type={field.type || "text"} placeholder={field.placeholder} name={field.name} value={val} onChange={onChange} required={field.required} step={field.step} />
            </FormGroup>
        );
    };
        ...
        <div className="p-8 max-w-[1600px] mx-auto relative font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-start">
                ...
                <div className="space-y-6">
                    {sections.filter(s => ['SMTP', 'Notifications', 'Cloud', 'Tokens', 'Secrets'].includes(s.id)).map(section => (
                ...
                <div className="space-y-6">
                    ...
                    {sections.filter(s => ['Pricing', 'Razorpay', 'Infrastructure', 'VercelRedis', 'MSG91'].includes(s.id)).map(section => (
                        <Card key={section.id} title={section.title} icon={section.icon}>
                            <form onSubmit={handleSubmit(section.id, section.keys)} className="flex flex-col">
                                {section.fields.map(field => renderField(field))}
                                <div className="mt-4"><Button loading={syncingSection === section.id}>Commit Config</Button></div>
                ...
                <div className="space-y-6">
                    {sections.filter(s => ['ApplicationMeta', 'AppleAuth', 'GoogleAuth', 'MetaAuth', 'UpstashRedis',].includes(s.id)).map(section => (
                        <Card key={section.id} title={section.title} icon={section.icon}>
                            <form onSubmit={handleSubmit(section.id, section.keys)} className="flex flex-col">
                                {section.fields.map(field => renderField(field))}
                                <div className="mt-4"><Button loading={syncingSection === section.id}>Commit Config</Button></div>
