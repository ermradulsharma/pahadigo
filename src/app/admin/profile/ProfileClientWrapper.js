...
export default function ProfileClientWrapper({ initialProfile }) {
    ...
    const [formData, setFormData] = useState({
        ...
        expertise: Array.isArray(initialProfile?.expertise) ? initialProfile.expertise.join(', ') : '',
        emergencyContacts: initialProfile?.emergencyContacts && initialProfile.emergencyContacts.length > 0 ? {
            name: initialProfile.emergencyContacts[0].name || '',
            phone: initialProfile.emergencyContacts[0].phone || '',
            relationship: initialProfile.emergencyContacts[0].relationship || ''
        } : { name: '', phone: '', relationship: '' },
        address: {
            addressLine1: initialProfile?.address?.addressLine1 || '',
            ...
            pincode: initialProfile?.address?.pincode || ''
        },
        preferences: {
            language: initialProfile?.preferences?.language || 'en',
            notifications: {
                email: initialProfile?.preferences?.notifications?.email ?? true,
                sms: initialProfile?.preferences?.notifications?.sms ?? true,
                push: initialProfile?.preferences?.notifications?.push ?? true
            }
        }
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await api.admin.profile.update(payload);
                ...
                <form onSubmit={handleSubmit} className="p-8 md:p-10 relative z-10 w-full">
                    ...
                    <FormSection title="Core Subroutine Information" icon={Terminal}>
                        ...
                        <div>
                            ...
                            <TextInput name="email" value={formData.email} disabled={true} />
                    ...
                    <FormSection title="Network Vectors & External Links" icon={Globe}>
                        ...
                        <div>
                            <InputLabel>Node: WhatsApp</InputLabel>
                            <TextInput name="socialLinks.whatsapp" type="url" value={formData.socialLinks.whatsapp} onChange={handleChange} placeholder="URI reference" />
                        </div>
                        <div>
                        ...
                        <div>
                            <InputLabel>Node: YouTube</InputLabel>
                            <TextInput name="socialLinks.youtube" type="url" value={formData.socialLinks.youtube} onChange={handleChange} placeholder="URI reference" />
                        </div>
                    </FormSection>
                    ...
                    <FormSection title="Emergency Protocol Routing" icon={Shield}>
                        <div>
                            ...
                            <TextInput name="emergencyContacts.name" value={formData.emergencyContacts.name} onChange={handleChange} />
                        </div>
                        <div>
                            <InputLabel>Target Comms (Phone)</InputLabel>
                            <TextInput name="emergencyContacts.phone" value={formData.emergencyContacts.phone} onChange={handleChange} />
                        </div>
                        <div>
                            <InputLabel>Target Relational Mesh (Relationship)</InputLabel>
                            <TextInput name="emergencyContacts.relationship" value={formData.emergencyContacts.relationship} onChange={handleChange} placeholder="e.g. Commander, Sibling" />
                        </div>
                    </FormSection>
                    ...
                    <FormSection title="Geographical Coordinates" icon={MapPin}>
                        <div>
                            ...
                            <TextInput name="address.addressLine1" value={formData.address.addressLine1} onChange={handleChange} />
                        </div>
                        <div>
