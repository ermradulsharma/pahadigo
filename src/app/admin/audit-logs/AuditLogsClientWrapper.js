...
export default function AuditLogsClientWrapper({ initialData, currentFilter, currentPage }) {
    const router = useRouter();
    const { logs, total, totalPages } = initialData;
    const [expandedLogId, setExpandedLogId] = useState(null);

    const handleFilterChange = (e) => {
        const newFilter = { ...currentFilter, [e.target.name]: e.target.value };
        const params = new URLSearchParams();
        if (newFilter.action) params.set('action', newFilter.action);
        if (newFilter.target) params.set('target', newFilter.target);
        params.set('page', '1');
    ...
    const handlePageChange = (newPage) => {
        ...
        if (currentFilter.target) params.set('target', currentFilter.target);
                ...
                <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-10">
                    ...
                    <div className="flex items-center gap-3 bg-gray-900/50 backdrop-blur-xl border border-white/10 p-2 rounded-2xl shadow-2xl">
                        <div className="relative group">
                            ...
                            <select name="action" value={currentFilter.action} onChange={handleFilterChange} className="pl-9 pr-10 py-2.5 bg-gray-950/50 border border-white/5 hover:border-white/10 text-sm font-medium text-gray-300 focus:ring-1 focus:ring-indigo-500/50 rounded-xl transition-all cursor-pointer appearance-none outline-none">
                        ...
                        <div className="relative group">
                            ...
                            <select name="target" value={currentFilter.target} onChange={handleFilterChange} className="pl-9 pr-10 py-2.5 bg-gray-950/50 border border-white/5 hover:border-white/10 text-sm font-medium text-gray-300 focus:ring-1 focus:ring-indigo-500/50 rounded-xl transition-all cursor-pointer appearance-none outline-none">
                    ...
                    {logs.length > 0 && (
                        <div className="p-6 border-t border-white/5 bg-gray-900/60 backdrop-blur-md flex flex-col sm:flex-row justify-between items-center gap-6">
                            <div className="text-[11px] font-mono text-gray-500 uppercase tracking-widest">
                                Processing <span className="text-indigo-400 px-1 font-bold">Block {currentPage}</span> / <span className="text-gray-400">{totalPages}</span>
