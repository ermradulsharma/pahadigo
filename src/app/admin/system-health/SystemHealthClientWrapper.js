...
export default function SystemHealthClientWrapper({ initialHealth }) {
    ...
    useEffect(() => {
        ...
        if (initialHealth?.memory) {
            setHistory([{
                time: new Date().toLocaleTimeString(),
                memory: initialHealth.memory.percentage,
                cpu: Math.round((initialHealth.cpu?.load?.[0] || 0) * 100) / 10
            }]);
        }
    }, []);
    ...
    useEffect(() => {
        if (initialHealth?.memory) {
            setHistory(prev => {
                ...
                return [...prev, newEntry].slice(-20); // Keep last 20 points
            });
        }
    }, [initialHealth]);
            ...
            <main className="max-w-[1500px] mx-auto px-6 lg:px-12 pt-12 pb-24 relative z-10">
                        ...
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    <div className={`w-2 h-2 rounded-full ${initialHealth?.status === 'healthy' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]' : 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.8)]'} animate-pulse`}></div>
                                    <span className="text-xs font-mono text-emerald-500/80 uppercase tracking-widest">Core Engine Status: {initialHealth?.status?.toUpperCase()}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <HealthCard 
                        ...
                        value={`${Math.floor((initialHealth?.uptime || 0) / 3600)}h ${Math.floor(((initialHealth?.uptime || 0) % 3600) / 60)}m`}
                    ...
                    <HealthCard 
                        title="Memory Load" 
                        value={`${initialHealth?.memory?.percentage || 0}%`}
                        subValue={`${formatBytes(initialHealth?.memory?.used)} / ${formatBytes(initialHealth?.memory?.total)}`}
                        Icon={Server}
                        color="text-indigo-400"
                        bg="bg-indigo-500/10"
                        progress={initialHealth?.memory?.percentage}
                    />
                    <HealthCard 
                        title="CPU Performance" 
                        value={`${initialHealth?.cpu?.cores || 0} Cores`}
                        subValue={initialHealth?.cpu?.model?.split(' ')[0]}
                        Icon={Cpu}
                    ...
                    <HealthCard 
                        ...
                        value={initialHealth?.activeUsers || 0}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="bg-gray-900/40 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl relative">
                        ...
                        <div className="space-y-6">
                            <DataRow label="Active Collections" value={initialHealth?.database?.collections} />
                            <DataRow label="Total Objects" value={initialHealth?.database?.objects?.toLocaleString()} />
                            <DataRow label="Physical Data Size" value={formatBytes(initialHealth?.database?.dataSize)} />
                            <DataRow label="Storage Allocation" value={formatBytes(initialHealth?.database?.storageSize)} />
                ...
                <div className="mt-8 bg-gray-900/20 border border-white/5 rounded-2xl p-6 flex flex-wrap gap-12 items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-bold text-slate-300">{initialHealth?.cpu?.model}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
