...
export default function DashboardClientWrapper({ initialStats, initialPackages }) {
    const router = useRouter();
    const [stats, setStats] = useState(initialStats);
    const [latestPackages, setLatestPackages] = useState(initialPackages.slice(0, 4));
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-8 max-w-400 mx-auto space-y-8 bg-[#0a0a0c] min-h-screen text-slate-300">
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 border-b border-white/10 pb-4">
                <div>
                    ...
                    <p className="text-xs font-mono text-slate-500 tracking-widest uppercase mt-2 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_#10b981]"></span> System Status: Online & Secured <span className="opacity-50">|</span> <span className="text-indigo-400">{currentTime ? currentTime.toLocaleTimeString() : '--:--:--'}</span>
                    </p>
                </div>
                <div className="flex gap-2">
                    <Link href="/admin/audit-logs" className="px-5 py-2.5 bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/20 hover:text-rose-300 rounded-lg transition-all shadow-[0_0_15px_rgba(244,63,94,0.1)] text-xs font-mono tracking-widest uppercase flex items-center gap-2"><ShieldAlert className="w-4 h-4" /> Security Reports</Link>
                    <Link href="/admin/vendors" className="px-5 py-2.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 hover:bg-indigo-500/20 hover:text-indigo-300 rounded-lg transition-all shadow-[0_0_15px_rgba(99,102,241,0.1)] text-xs font-mono tracking-widest uppercase flex items-center gap-2"><Briefcase className="w-4 h-4" /> Vendor Directory</Link>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-4 items-center bg-[#111116] p-4 rounded-xl border border-white/5 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                <div className="text-xs font-mono text-slate-500 tracking-widest uppercase flex items-center gap-2 mr-4"><Terminal className="w-4 h-4 text-emerald-400" /> Quick Actions:</div>
                <button onClick={() => showToast("Vendor Mass Mailing feature will be rolling out in v2.1", "info")} className="text-[10px] font-mono tracking-widest uppercase px-4 py-2 bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 rounded hover:bg-indigo-500/20 transition-all flex items-center gap-2 border-l-2 border-l-indigo-500"><MessageSquare className="w-3 h-3" /> Message All Vendors</button>
                <Link href="/admin/system-health" className="text-[10px] font-mono tracking-widest uppercase px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded hover:bg-emerald-500/20 transition-all flex items-center gap-2 border-l-2 border-l-emerald-500"><Activity className="w-3 h-3" /> Check System Health</Link>
                <Link href="/admin/payments" className="text-[10px] font-mono tracking-widest uppercase px-4 py-2 bg-pink-500/10 border border-pink-500/30 text-pink-400 rounded hover:bg-pink-500/20 transition-all flex items-center gap-2 border-l-2 border-l-pink-500"><TrendingUp className="w-3 h-3" /> Generate Revenue Report</Link>
                <button onClick={() => showToast("✅ Master Application Cache Cleared successfully from CDN edge routing.")} className="text-[10px] font-mono tracking-widest uppercase px-4 py-2 bg-amber-500/10 border border-amber-500/30 text-amber-400 rounded hover:bg-amber-500/20 transition-all flex items-center gap-2 border-l-2 border-l-amber-500"><Zap className="w-3 h-3" /> Clear Application Cache</button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-4">
                    ...
                    <div className="flex-1 space-y-6 flex flex-col justify-center">
                        <div onClick={() => router.push('/admin/system-health')} className="relative group cursor-pointer">
                            ...
                            <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                                <div className="h-full bg-indigo-500 rounded-full shadow-[0_0_10px_rgba(99,102,241,0.8)] group-hover:bg-indigo-400 transition-colors" style={{ width: `${stats.systemHealth?.dbLoad || 0}%` }}></div>
                            </div>
                        </div>
                        ...
                        <div onClick={() => router.push('/admin/system-health')} className="relative group cursor-pointer">
                            ...
                            <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.8)] group-hover:bg-emerald-400 transition-colors" style={{ width: `${Math.min((stats.systemHealth?.latency || 0) / 2, 100)}%` }}></div>
                            </div>
                        </div>
                ...
                <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="bg-[#111116] border border-white/10 p-6 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] flex flex-col h-87.5">
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        {(!stats.topTerritories || stats.topTerritories.length === 0) ? (
                            <div className="text-center py-8 text-[10px] font-mono text-slate-600 uppercase tracking-widest">No territory data</div>
                            ...
                            <div onClick={() => router.push('/admin/categories')} key={idx} className="flex items-center gap-4 hover:bg-white/5 p-1 rounded transition-colors cursor-pointer group">
                                ...
                                <div className="flex-1 h-2.5 bg-white/5 border border-white/10 rounded overflow-hidden">
                                    <motion.div initial={{ width: 0 }} whileInView={{ width: `${t.load}%` }} transition={{ duration: 1, delay: 0.2 + idx * 0.1 }} className={`h-full ${t.color} opacity-80 group-hover:opacity-100 shadow-[0_0_10px_currentColor]`} />
                                </div>
                ...
                <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="bg-[#111116] border border-white/10 p-6 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] flex flex-col relative overflow-hidden h-87.5">
                    ...
                    <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar relative z-10 font-mono">
                        {(!stats.systemActivity || stats.systemActivity.length === 0) ? (
                            <div className="text-center py-8 text-[10px] font-mono text-slate-600 uppercase tracking-widest">No recent logs</div>
                            ...
                            <div onClick={() => router.push('/admin/audit-logs')} key={i} className="flex items-start gap-3 border-b border-white/5 pb-2 last:border-0 last:pb-0 group cursor-pointer hover:bg-white/5 p-1 rounded transition-colors -mx-1">
                                ...
                                <div className={`px-1.5 py-0.5 rounded text-[8px] tracking-widest font-bold uppercase shadow-sm ${log.status === 'error' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : log.status === 'warning' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : log.status === 'success' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'}`}>{log.type}</div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2, duration: 0.5 }} className="pt-4">
                <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4 border-t border-white/5 pt-8">
                    <h2 className="text-xl font-bold text-white flex items-center gap-3"><Activity className="text-indigo-400 w-6 h-6" /> Analytics Overview</h2>
                                ...
                                <div className="h-64">
                                    <ResponsiveContainer width="100%" height={256}>
                                        <LineChart data={analyticsData.userGrowth} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                            ...
                                            <RechartsTooltip cursor={{ stroke: '#ffffff1a', strokeWidth: 1 }} labelFormatter={(label) => new Date(label).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} contentStyle={{ borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', backgroundColor: '#0a0a0c', color: '#cbd5e1', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.5)' }} />
                                            <Legend verticalAlign="top" align="right" iconType="circle" onClick={toggleLine} wrapperStyle={{ paddingBottom: '20px', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer' }} />
                                            <Line type="linear" dataKey="travellers" stroke="#60a5fa" strokeWidth={2} hide={hiddenLines.includes('travellers')} dot={{ r: 3, fill: '#60a5fa', strokeWidth: 0 }} activeDot={{ r: 5, strokeWidth: 0, fill: '#60a5fa', stroke: '#fff', strokeWidth: 2 }} name="Travellers" animationDuration={1000} />
                                            <Line type="linear" dataKey="vendors" stroke="#10b981" strokeWidth={2} hide={hiddenLines.includes('vendors')} dot={{ r: 3, fill: '#10b981', strokeWidth: 0 }} activeDot={{ r: 5, strokeWidth: 0, fill: '#10b981', stroke: '#fff', strokeWidth: 2 }} name="Vendors" animationDuration={1000} />
                                            <ReferenceLine y={80} label={{ value: 'GOAL', position: 'insideRight', fill: '#fb923c', fontSize: 10, fontStyle: 'italic' }} stroke="#fb923c" strokeDasharray="10 10" strokeWidth={1} opacity={0.3} />
                                        </LineChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
                                ...
                                {(stats.recentBookings || []).map((b, i) => (
                                    <tr onClick={() => router.push('/admin/bookings')} key={b?._id || i} className="group hover:bg-white/5 cursor-pointer transition-colors">
                                        <td className="p-2 border-r border-white/5">
                                            <div className="text-[10px] font-mono font-bold text-indigo-400">{b?.bookingCode || 'PH-XXXXX'}</div>
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} className="pt-8 border-t border-white/5">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-sm font-mono tracking-widest text-pink-400 uppercase flex items-center gap-2"><PackageSearch className="w-4 h-4" /> Recently Added Packages</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    {latestPackages.length === 0 && <div className="col-span-full py-16 text-center text-xs font-mono tracking-widest uppercase text-slate-500 bg-[#111116] rounded-2xl border border-white/5 border-dashed">No packages added yet.</div>}
                </div>
            </motion.div>
...
function StatCard({ title, value, colorClass, bgClass, borderClass, glowClass, Icon, href, delay = 0, pulse = false }) {
    const CardContent = (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay, duration: 0.5 }}
            ...
            <div className="absolute top-4 right-4 opacity-5 group-hover:opacity-10 transform group-hover:scale-110 transition-all duration-500"><Icon className={`w-16 h-16 ${colorClass}`} /></div>
            <div className="relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-start mb-4">
                    <div className={`w-10 h-10 ${bgClass} border ${borderClass} rounded-xl flex items-center justify-center backdrop-blur-md`}><Icon className={`w-5 h-5 ${colorClass}`} /></div>
                    {pulse && <span className="flex h-2.5 w-2.5 relative"><span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${bgClass.replace('/10', '')} opacity-75`}></span>
                        <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${bgClass.replace('/10', '')}`}></span></span>}
                </div>
