...
export default function MarketingClientWrapper({ initialBanners, initialCoupons }) {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto min-h-[80vh]">
      ...
      >
        {activeTab === 'banners' ? <BannersManager initialBanners={initialBanners} /> : <CouponsManager initialCoupons={initialCoupons} />}
      </motion.div>
    </div>
  );
}

function BannersManager({ initialBanners }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(true);
    try {
      const payload = { ...newBanner, position: newBanner.position || 0 };
      const data = await api.admin.marketing.banners.add(payload);
      if (data.success) {
      ...
      <div className="lg:col-span-1">
        <div className="bg-[#111116] p-6 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] border border-white/10 flex flex-col relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
            <div>
              <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-widest mb-1.5">Title <span className="text-rose-500">*</span></label>
              <input
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-[10px] font-mono text-slate-500 mt-2 truncate max-w-sm flex items-center gap-1.5">
                <Link2 className="w-3 h-3 text-slate-600" /> {banner.linkUrl || 'Empty Vector'}
              </div>
            </div>
            <button
...
function CouponsManager({ initialCoupons }) {
  const router = useRouter();
  const [coupons, setCoupons] = useState(initialCoupons);
  const [newCoupon, setNewCoupon] = useState({ code: '', discountType: 'percentage', discountValue: 0, maxUses: 0, expiryDate: '' });
  const [isSaving, setIsSaving] = useState(false);
  const toast = useToast();
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSaving(true);
    try {
      const payload = { ...newCoupon, discountValue: newCoupon.discountValue || 0, maxUses: newCoupon.maxUses || 0 };
      const data = await api.admin.marketing.coupons.create(payload);
      if (data.success) {
        setCoupons(prev => [data.data.coupon, ...prev]);
        setNewCoupon({ code: '', discountType: 'percentage', discountValue: 0, maxUses: 0, expiryDate: '' });
        router.refresh();
      }
    } catch (e) {
        ...
        <div className="bg-[#111116] p-6 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] border border-white/10 flex flex-col relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-widest mb-1.5">Value <span className="text-rose-500">*</span></label>
                <input
                  ...
                  onChange={e => setNewCoupon({ ...newCoupon, discountValue: e.target.value ? parseFloat(e.target.value) : 0 })}
                />
              </div>
            </div>
            <div>
              <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                <DollarSign className="w-3 h-3 text-slate-500" /> Max Uses
              </label>
              <input
                ...
                onChange={e => setNewCoupon({ ...newCoupon, maxUses: e.target.value ? parseInt(e.target.value) : 0 })}
              />
            </div>
            <div>
              <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                <Calendar className="w-3 h-3 text-slate-500" /> Date of Expiry (Optional)
              </label>
              <input
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="text-sm font-medium text-slate-200 flex items-center gap-2">
                  ...
                  Reduction: <span className="font-mono font-bold text-indigo-400">{coupon.discountValue}{coupon.discountType === 'percentage' ? '%' : '₹'}</span>
                </div>
                <div className="text-[10px] text-slate-500 mt-2 font-mono uppercase tracking-widest flex items-center gap-1.5">
                  <Calendar className="w-3 h-3" /> Terminus: {coupon.expiryDate ? new Date(coupon.expiryDate).toLocaleDateString() : 'N/A'}
                </div>
              </div>

              <div className="flex items-center gap-6 relative z-10 pl-4 border-l border-white/5">
                <div className="text-center group-hover:scale-105 transition-transform">
                  <div className="text-3xl font-mono font-black text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]">{coupon.maxUses || '∞'}</div>
                  <div className="text-[9px] uppercase tracking-widest text-slate-500 font-mono mt-1">Max Uses</div>
                </div>
                <button
