...
export default function PoliciesClientWrapper({ initialPolicies }) {
  ...
  useEffect(() => {
    if (isMounted) {
      const current = initialPolicies.find(p => p.target === target && p.type === type);
      setContent(current ? current.content : '');
    }
  }, [target, type, isMounted, initialPolicies]);
