        ...
        const fetchData = async () => {
            try {
                const [statsRes, pkgRes] = await Promise.all([
                    api.admin.stats.getStats(),
                    api.admin.packages.getAll()
                ]);
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-8 max-w-[1600px] mx-auto space-y-8 bg-[#0a0a0c] min-h-screen text-slate-300">
            ...
            <div className="flex flex-wrap gap-4 items-center bg-[#111116] p-4 rounded-xl border border-white/5 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                ...
                <button onClick={() => showToast("System Health Monitoring Interface under construction. ETA: Next Release.", "info")} className="text-[10px] font-mono tracking-widest uppercase px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded hover:bg-emerald-500/20 transition-all flex items-center gap-2 border-l-2 border-l-emerald-500"><Activity className="w-3 h-3" /> Check System Health</button>
            ...
            <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} className="pt-8 border-t border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {loading ? ([1, 2, 3, 4].map(i => <div key={i} className="h-64 bg-white/5 border border-white/10 rounded-2xl animate-pulse delay-75"></div>)) : latestPackages.map((pkg, idx) => (
