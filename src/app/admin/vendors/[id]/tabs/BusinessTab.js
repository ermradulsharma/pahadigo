...
import { DetailItem, MainPanelCard, VendorHeader, UnifiedStatusMenu, SidebarCard, StatusBadge, Modal } from '@/components/admin/VendorUIFragments.js';
import { User, Layers, Building2, ShieldCheck, MapPin, Landmark, Check as CheckIcon } from 'lucide-react';
...
export default function BusinessTab({ vendor, setVendor, id, activeTab, setActiveTab, onRefresh }) {
    ...
    const verifyDocument = async (field, status, reason = '', index = null) => {
        setSaving(true);
        try {
            await api.admin.compliance.verifyDocument({ vendorId: id, documentField: field, status, reason, index });
            if (onRefresh) onRefresh();
            toast(`Document ${status} successfully.`, "success");
        } catch (e) {
            toast(e.message || "Error updating document status.", "error");
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            const data = await api.admin.vendors.update(id, formData);
            setVendor(data.data.vendor);
            setIsEditing(false);
