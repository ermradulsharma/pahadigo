...
import { Calendar, Search, Mail, Phone, CreditCard, Eye, Undo2, ExternalLink } from 'lucide-react';
...
export default function BookingsPage() {
    ...
    const fetchBookings = async (page = 1, status = 'all') => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/bookings?page=${page}&limit=10&status=${status}`, {
                ...
                <div className="group relative flex flex-col gap-1 cursor-help">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="group/details relative">
                            ...
                            {/* Simple & Clean Price Breakdown Popover */}
                            <div className="absolute left-full top-1/2 -translate-y-1/2 ml-4 p-4 bg-[#0a0a0c] border border-white/10 rounded-lg shadow-2xl opacity-0 invisible group-hover/details:opacity-100 group-hover/details:visible transition-all duration-200 z-[100] w-56 backdrop-blur-md">
                                <div className="text-[11px] font-mono space-y-2">
                                    ...
                                    <div className="flex justify-between text-slate-400">
                                        <span>Subtotal</span>
                                        <span className="text-white">₹{(b.pricing?.subTotal || 0).toLocaleString()}</span>
                                    </div>

                                    <div className="flex justify-between text-indigo-400">
                                        <span>Service Fee</span>
                                        <span>+₹{(b.pricing?.serviceFee || 0).toLocaleString()}</span>
                                    </div>

                                    <div className="flex justify-between text-rose-400">
                                        <span>Discounts</span>
                                        <span>-₹{((b.pricing?.discount || 0) + (b.pricing?.couponAmount || 0)).toLocaleString()}</span>
                                    </div>

                                    <div className="flex justify-between text-slate-400">
                                        <span>Tax ({b.pricing?.taxRate}%)</span>
                                        <span className="text-white">₹{(b.pricing?.tax || 0).toLocaleString()}</span>
                                    </div>

                                    <div className="border-t border-white/10 pt-2 mt-2 flex justify-between text-emerald-400 font-bold text-xs">
                                        <span>Total</span>
                                        <span>₹{(b.pricing?.total || 0).toLocaleString()}</span>
                                    </div>
                                </div>
                                {/* Simple Arrow */}
            ...
            render: (b) => (
                <div className="flex items-center gap-2">
                    ...
                    <div className="group/details relative">
                        ...
                        {/* Timeline Popover (Matching Breakdown Design) */}
                                ...
                                <div className="relative space-y-4 border-l border-indigo-500/20 ml-1.5 pl-4 max-h-64 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden pr-2">
                                    {(b.timeline && b.timeline.length > 0 ? b.timeline : [{ status: 'LOG_INIT', timestamp: b.createdAt, remarks: 'System record initialized.' }]).map((event, idx) => (
                                        <div key={idx} className="relative group/node mb-4 last:mb-0">
                                            <div className="absolute -left-[21.5px] top-1 w-2.5 h-2.5 rounded-full bg-[#0a0a0c] border border-indigo-500/50 flex items-center justify-center">
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto">
            ...
            <CyberTable
                ...
                columns={columns}
                itemsPerPage={10}
                totalItems={totalMetadata.total}
                externalCurrentPage={currentPage}
                onPageChange={(page) => setCurrentPage(page)}
                searchTerm={searchQuery}
                searchKeys={['user.name', 'user.email', 'vendor.businessName', 'vendor.ownerName', '_id', 'serviceDetails.title', 'serviceType']}
