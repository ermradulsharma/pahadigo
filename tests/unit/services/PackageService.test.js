...
describe('PackageService Test Suite', () => {
    ...
    it('should initialize an empty catalog for a new vendor', async () => {
        ...
        expect(catalog.business.toString()).toBe(vendorId.toString());
    ...
    describe('Baseline Updates (Pricing & Availability)', () => {
        it('should update baseline pricing and availability fields in a single call', async () => {
            const initialItem = await PackageService.addServiceItem(vendorId, 'trekking', {
                title: 'Sample Trek', description: 'desc', pricing: { pricePerPerson: 100 },
                location: { address: 'test' }, availability: { totalSlots: 5 }
            });
            const itemId = initialItem.id;
            const updates = { pricing: { pricePerPerson: 150 }, availability: { totalSlots: 20 }, isActive: false };
            const updatedItem = await PackageService.updateServiceItem(vendorId, 'trekking', itemId, updates);

            expect(Number(updatedItem.pricing.pricePerPerson)).toBe(150);
            expect(updatedItem.availability.totalSlots).toBe(20);
